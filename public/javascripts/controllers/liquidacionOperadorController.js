reportMedias.controller('liquidacionOperadorController',function($scope, $http,$cookies,$location, $rootScope, Title){
	$scope.filtroURL = 'interior/liquidacion_operador/filtro.html';
	$scope.tablaURL = 'interior/liquidacion_operador/tabla_liquidacion.html';
	$scope.formLiquidacion = {id_pais:1}; // ID CHILE
	$scope.operadores = {};
	$scope.formPath = {};
	$scope.pais = {};
	...
	$scope.buscarDatos = function(){
		...
		$http.post('/liquidacionOperador', $scope.formLiquidacion).then(function(result){
			...
			$scope.tablaURL = 'interior/liquidacion_operador/tabla_liquidacion.html?upd='+Math.random();
