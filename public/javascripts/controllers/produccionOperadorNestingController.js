reportMedias.controller('produccionOperadorNestingController',function($scope, $http,$cookies,$location,Notification,Title){
	$scope.operador=$cookies.operadora;
	$scope.tablaURL = 'interior/produccion_operador_nesting/tabla.html';
	$scope.filtroURL = "interior/produccion_operador_nesting/filtro.html";
	...
	$scope.verProduccion = function(){
		...
		$http.post("/getProduccionOperador",$scope.formProduccion).then(function(result){
			...
			$scope.tablaURL = 'interior/produccion_operador_nesting/tabla.html?upd='+Math.random();
