reportMedias.controller('monitorGestionesController',function($scope, $http,$cookies,$location,Notification,GlobalServices,Title){
	...
	$scope.dataTable             = "";
	$scope.gestionesUrl          = 'interior/monitor_gestiones/tabla.html';
	$scope.clientesVibox         = {};
	...
	$scope.traerMedias = function(){
		$http.post('/monitor/gestionesProduccionReal')
		.then(function(result){
			...
			if(cant_days > 0)
				$scope.gestionesUrl = 'interior/monitor_gestiones/tabla_fechas.html?'+((1+Math.random())*1000);
			else
				$scope.gestionesUrl = 'interior/monitor_gestiones/tabla.html?'+((1+Math.random())*1000);

			setTimeout($scope.configDataTable,1000);
		});
	}
	$scope.traerMedias();
	setInterval($scope.traerMedias,5000);
